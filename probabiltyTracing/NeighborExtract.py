...
# NeighborExtract.m:17
                    dataset[arange(1,3),kk]=concat([[Indexx(ii)],[Indeyy(jj)],[Indezz(ij)]])
...
# NeighborExtract.m:29
        aver0=(dot(ww,dataset(4,arange()).T)) / sum(ww)
# NeighborExtract.m:32
        aver0=dot(0.2,aver0) + dot(0.8,sum(dataset(4,arange()))) / (dot(dot(length(Indexx),length(Indeyy)),length(Indezz)))
