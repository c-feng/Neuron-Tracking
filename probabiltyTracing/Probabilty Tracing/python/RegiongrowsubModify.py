            ...
            for ii in arange(1,length(Indexx)).reshape(-1):
                for jj in arange(1,length(Indeyy)).reshape(-1):
                    for ij in arange(1,length(Indezz)).reshape(-1):
                        if ImgBinary[int(Indexx[ii]),int(Indeyy[jj]),int(Indezz[ij])] == Labeflag:
                            kk=kk + 1
# RegiongrowsubModify.m:18
                            DataP[:, kk]=concat([Indexx[ii],Indeyy[jj],Indezz[ij]])[:, 0]
# RegiongrowsubModify.m:19
                            ImgBinary[int(Indexx[ii]),int(Indeyy[jj]),int(Indezz[ij])]=0
# RegiongrowsubModify.m:20
        if kk > 0:
            datap=DataP[:, arange(1,kk)]
            if datap.shape[0]==1:
                datap = datap.reshape([3, 1])
...
#             Dataset[:,arange(kk + 1,kk + size(Currdata,2))]=Currdata
