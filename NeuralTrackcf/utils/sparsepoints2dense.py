...
def process_ins2dense(swcs_path):
    names = os.listdir(swcs_path)
    mask = np.zeros([300, 300, 300])
    for name in names:
        # swc_path = os.path.join(swcs_path, name)
        label = str_to_float(name.split('.')[0].split('_')[-1])
        swc_num = np.array(psc(swcs_path, name.split('.')[0]))
        c_e = find_cross_end(swc_num)
        m = sparse2dense_Loop(swc_num, shape=[300, 300, 300], label=label)
        mask[m>0] = m[m>0]
...
#    m = Connect_2_points(p1=[172, 136, 3], p2=[173, 138, 4])
    ...
    import time
    start = time.time()
    swcs_path = r"C:\Users\Administrator\Desktop\Syn_chaos_300_5_2_4_6_000\swcs_test"
    mask = process_ins2dense(swcs_path)
    tifffile.imsave(os.path.join(swcs_path, 'gts_dense.tif'), (mask).astype(np.int))
    print("Using time: {}s.".format(time.time()-start))