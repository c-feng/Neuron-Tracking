...
def calculate_means(pred, gt, n_objects, usegpu):
    """pred: bs, height * width * depth, n_filters
       gt: bs, height * width * depth, n_instances"""

    bs, n_loc, n_filters = pred.size()
    n_instances = gt.size(2)
    
    max_n_objects = n_instances
    ...
    for i in range(bs):
        ...
        #print("num of logitals",_gt_expanded_sample.sum(0))  # n_objects, n_filters 
...
def discriminative_loss(input, target, n_objects,
                         delta_v, delta_d, norm, usegpu):
    """input: bs, n_filters, fmap, fmap, fmap
       target: bs, n_instances, fmap, fmap, fmap
       n_objects: bs"""
    ...
    input = input.permute(0, 2, 3, 4, 1).contiguous().view(
        bs, height * width * depth, n_filters)
    target = target.permute(0, 2, 3, 4, 1).contiguous().view(
        bs, height * width * depth, n_instances)

    cluster_means = calculate_means(
        input, target, n_objects, usegpu)
    ...
    #print(var_term,dist_term,reg_term)
    return loss
...
class DiscriminativeLoss(_Loss):
    ...
    def forward(self, input, target, n_objects):
        return discriminative_loss(input, target, n_objects,
