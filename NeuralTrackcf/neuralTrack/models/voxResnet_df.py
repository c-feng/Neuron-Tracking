...
class VoxResnet(nn.Module):
    def __init__(self,in_dims,num_classes):
        ...
        self.upsample_0 = nn.Conv3d(32,3,3,1,1)
        #self.upsample_0 = nn.Upsample()
        self.classifier_0 = nn.Conv3d(3,3,1,1,0)

        self.upsample_1 = nn.ConvTranspose3d(64,3,2,2,0)
        #self.upsample_1 = nn.Upsample(scale_factor = 2)
        self.classifier_1 = nn.Conv3d(3,3,1,1,0)
        #self.classifier_1 = nn.Conv3d(64,num_classes,1,1,0)

        self.upsample_2 = nn.ConvTranspose3d(64,3,4,4,0)
        #self.upsample_2 = nn.Upsample((4,4,4))
        #self.upsample_2 = nn.Upsample(scale_factor = 4)
        self.classifier_2 = nn.Conv3d(3,3,1,1,0)
        #self.classifier_2 = nn.Conv3d(64,num_classes,1,1,0)

        self.upsample_3 = nn.ConvTranspose3d(64,3,8,8,0)
        #self.upsample_3 = nn.Upsample((8,8,8))
        #self.upsample_3 = nn.Upsample(scale_factor = 8)
        self.classifier_3 = nn.Conv3d(3,3,1,1,0)
        #self.classifier_3 = nn.Conv3d(64,num_classes,1,1,0)
        self.s_0 = nn.Conv3d(3,2,1,1,0)
        self.s_1 = nn.Conv3d(3,2,1,1,0)
        self.s_2 = nn.Conv3d(3,2,1,1,0)
        self.s_3 = nn.Conv3d(3,2,1,1,0)
        self.s = nn.Conv3d(3,2,1,1,0) 
        self.n = nn.functional.normalize
        self.reset_params()
    def forward(self,img):
        x = img
        ...
        output_3 = self.n(output_3)
        
        output = output_0 + output_1 + output_2 + output_3
        output = self.n(output)

        s = self.s(self.relu(output))
        s_0 = self.s_0(self.relu(output_0))
        s_1 = self.s_1(self.relu(output_1))
        s_2 = self.s_2(self.relu(output_2))
        s_3 = self.s_3(self.relu(output_3))
        return (output,s),(output_0,s_0),(output_1,s_1),(output_2,s_2),(output_3,s_3)
