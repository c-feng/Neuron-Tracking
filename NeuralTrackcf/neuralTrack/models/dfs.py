...
class DFS(nn.Module):
    def __init__(self,in_channels,num_classes):
        ...
        self.vox_0 = make_layers([in_channels,8,8])
        self.maxPool_0 = nn.MaxPool3d(2,2)
        #self.maxPool_0 = nn.Sequential(nn.Conv3d(16,16,3,2,1),\
        #        nn.BatchNorm3d(16),nn.LeakyReLU(1e-5))
        

        self.voxRes_1 = VoxRes([8,8,8])
        self.voxRes_2 = VoxRes([8,16,16])
        
        self.maxPool_2 = nn.MaxPool3d(2,2)
        #self.maxPool_2 = nn.Conv3d(64,64,3,2,1)
        #self.maxPool_2 = nn.Sequential(nn.Conv3d(64,64,3,2,1),\
        #        nn.BatchNorm3d(64),nn.LeakyReLU(1e-5))

        self.voxRes_3 = VoxRes([16,16,16])
        self.voxRes_4 = VoxRes([16,16,16])
        self.maxPool_4 = nn.MaxPool3d(2,2)
        #self.maxPool_4 = nn.Conv3d(64,64,3,2,1)
        #self.maxPool_4 = nn.Sequential(nn.Conv3d(64,64,3,2,1),\
        #        nn.BatchNorm3d(64),nn.LeakyReLU(1e-5))
        
        self.voxRes_5 = VoxRes([16,32,32])
        self.voxRes_6 = VoxRes([32,32,32])

        self.side_upsample_6 = nn.Sequential(nn.ConvTranspose3d(32,16,2,2,0),\
                nn.BatchNorm3d(16),nn.LeakyReLU(1e-5))
        self.side_output_6 = nn.Sequential(nn.Conv3d(32,2,1,1,0),nn.ConvTranspose3d(2,2,8,8,0))

        self.side_filter_4 = nn.Sequential(nn.Conv3d(16,16,1,1,0),\
                nn.BatchNorm3d(16),nn.LeakyReLU(1e-5))
        self.side_upsample_4 = nn.Sequential(nn.ConvTranspose3d(16,16,2,2,0),\
                nn.BatchNorm3d(16),nn.LeakyReLU(1e-5))
        self.side_output_4 = nn.Sequential(nn.Conv3d(16,2,1,1,0),nn.ConvTranspose3d(2,2,4,4,0)) 

        self.side_filter_2 = nn.Sequential(nn.Conv3d(16,16,1,1,0),\
                nn.BatchNorm3d(16),nn.LeakyReLU(1e-5))
        self.side_output_2 = nn.Sequential(nn.Conv3d(16,2,1,1,0),nn.ConvTranspose3d(2,2,2,2,0)) 
