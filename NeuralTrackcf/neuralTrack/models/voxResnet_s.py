...
class VoxResnet(nn.Module):
    def __init__(self,in_dims,num_classes):
        ...
        self.df_0 = nn.Conv3d(2,3,1,1,0)
        self.df_1 = nn.Conv3d(2,3,1,1,0)
        self.df_2 = nn.Conv3d(2,3,1,1,0)
        self.df_3 = nn.Conv3d(2,3,1,1,0)
        self.df = nn.Conv3d(2,3,1,1,0) 
    ...
    def forward(self,img):
        ...
        df = self.df(self.relu(output))
        df = self.n(df)

        df_0 = self.df_0(self.relu(output_0))
        df_0 = self.n(df_0)

        df_1 = self.df_1(self.relu(output_1))
        df_1 = self.n(df_1)

        df_2 = self.df_2(self.relu(output_2))
        df_2 = self.n(df_2)

        df_3 = self.df_3(self.relu(output_3))
        df_3 = self.n(df_3)
        return (df,output),(df_0,output_0),(df_1,output_1),(df_2,output_2),(df_3,output_3)
...
if __name__ == "__main__":
    ...
    print(output[0].size(),output_0[-1].size())
