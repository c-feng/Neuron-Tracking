...
from trainer import DirectFieldTrainer
from evaluator import DirectFieldEvaluator
from dataset import DatasetSyn, Compose, ToTensor, DatasetEvaluate, DatasetReal
...
INIT_WEIGHT = "/media/fcheng/NeuralTrackcf/logs/fov64_DF/model_best.pth.tar"
...
parser.add_argument('--logs-dir', type=str, metavar='PATH',
                                 default=osp.join(working_dir, 'logs/realData/fov64_DF_finetune'))
...
parser.add_argument('--fov_shape', type=list, default=[64,64,64])
parser.add_argument("--sample_shape", type=list, default=[64, 64, 64])
...
# scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,[50,75,100,115,125],gamma = 0.5)
...
trainer = DirectFieldTrainer(model, criterion, args.logs_dir, metric_func, args.fov_shape, device=device)
evaluator = DirectFieldEvaluator(model, criterion, metric_func, args.fov_shape, device=device)
